...
Starting out logic to build characters. 

This is not the final stage, but work in progress. 
...
Write-Host Building Character -NoNewLine
Sleep 1
...
Write-Host "Your Characters Stats:"
str
dex
con
int
...
Write-Host "Where does your character fit in?"
sleep 1
Write-Host "
    Strength
...
$tick3 = ($tick1+$tick)
$tick4 = ($tick1-$tick)
$a = (Get-Random -Minimum 1 -Maximum 6 -SetSeed $tick)
$b = (Get-Random -Minimum 1 -Maximum 6 -SetSeed $tick1)
$c = (Get-Random -Minimum 1 -Maximum 6 -SetSeed $tick3)
$d = (Get-Random -Minimum 1 -Maximum 6 -SetSeed $tick4)
if ($a -eq $b -or $a -eq $c -or $a -eq $d) { 
    $tick = ([Environment]::TickCount)
    $a = (Get-Random -Minimum 1 -Maximum 6 -SetSeed $tick)
}
if ($b -eq $a -or $b -eq $c -or $b -eq $d) { 
    $tick = ([Environment]::TickCount)
    $b = (Get-Random -Minimum 1 -Maximum 6 -SetSeed $tick)
}
if ($c -eq $a -or $c -eq $b -or $c -eq $d) { 
    $tick = ([Environment]::TickCount)
    $c = (Get-Random -Minimum 1 -Maximum 6 -SetSeed $tick)
}
if ($d -eq $a -or $d -eq $c -or $d -eq $b) { 
    $tick = ([Environment]::TickCount)
    $d = (Get-Random -Minimum 1 -Maximum 6 -SetSeed $tick)
}

if ($a -lt $b -and $a -lt $c -and $a -lt $d) { $f = $b+$c+$d}
if ($b -lt $a -and $b -lt $c -and $b -lt $d) { $f = $a+$c+$d}
if ($c -lt $a -and $c -lt $b -and $c -lt $d) { $f = $a+$b+$d}
if ($d -lt $a -and $d -lt $c -and $d -lt $b) { $f = $a+$b+$c}
$f = $a+$b+$c+$d

Write-Host "Strength: "$f
}
